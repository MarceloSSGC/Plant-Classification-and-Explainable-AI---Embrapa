...
EXP_TYPE = "MTV_5_BANDS"
EXP_DIR = f"/home/u14696181/Documents/Datasets/Embrapa_Experimentos/Results/{EXP_TYPE}/{EXP_NAME}"


ABL_NAME = "Ablation_02"
ABL_DIR = f"/home/u14696181/Documents/Datasets/Embrapa_Experimentos/Ablation/{ABL_NAME}"

#======================================================================
# All Models

EXP_LIST = os.listdir(f"/home/u14696181/Documents/Datasets/Embrapa_Experimentos/Results/{EXP_TYPE}")

EXP_LIST = sorted([x for x in EXP_LIST if "EPOCHS_1" not in x])
print(f"len(EXP_LIST): {len(EXP_LIST)}")
...
for EXP_NAME in EXP_LIST:   # EXP_NAME = EXP_LIST[3]
    ...
    do_ablation_02(TEST_DATA_DIR, EXP_DIR, ABL_DIR)
...
def do_ablation_02(
    ...
    if not os.path.isfile(df_all_dir) or not os.path.isfile(fig_dir):
        ...
        cases_list = ([3, 4], [0, 1, 2], 
                      [1, 2, 3, 4], [0, 2, 3, 4], [0, 1, 3, 4], [0, 1, 2,  4], [0, 1, 2,  3])

        name_cases_list = ["Only_RGB", "Only_NIR_RE", "Only_Blue", "Only_Green", 
                           "Only_Red", "Only_NIR", "Only_RedEdge"]
        ...
        for i, bands in enumerate(cases_list):   # i, bands = 0, cases_list[0]

            my_transform = define_T(bands)

            test_dataset = WeedDataset_Transform(TEST_DIR, transform=my_transform)

            print(test_dataset[23][0].std(axis=(1, 2)))
        ...
        fig_acc, ax_acc = plot_ablation_2_metric(df_all, title=f"Spectral Band Ablation Study - {model_name}")
