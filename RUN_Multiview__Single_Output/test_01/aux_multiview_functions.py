...
def one_view_function(img_5b, type_view, funct, param):
    ...
    if "which_band" not in param.keys():
        raise ValueError("which_band no in param")
...
def mulview_function(img_5b, trans_list, plot_img=False):
    ...
    for ith_trans in trans_list: # ith_trans = trans_list[k]    k=4

        ith_view = one_view_function(img_5b, **ith_trans)
    ...
    for i, especie in enumerate(especies):    # i, especie = 0, especies[0]
        ...
        for file_name in files: # file_name = files[0]
            ...
            if not os.path.isfile(file_dir):
                ...
                img_multiview = mulview_function(img_5b, trans_list)
