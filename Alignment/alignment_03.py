...
#======================================================================
#======================================================================
def load_norm_multiband_image(directory, image_name, n_bands=5):
    """
    Carrega as bandas separadas de uma imagem e retorna um array normalizado
    (min-max real, reversível sem perdas).
    ...
    for b in range(1, n_bands + 1):
        ...
        v_min = float(band_float.min())
        v_max = float(band_float.max())
...
NEW_DATA_DIR = "/media/marcelo/HD_8t/Marcelo__Seagate_8tb/Embrapa/Embrapa_Experimentos/Datasets/PlantaDaninha_BoaVista_Aligned_ecc_affine_interch_45_cen_5"
...
# Trocar 4 pela 5, centralizar red edge
...
def align_multispectral_ecc(
    ...
    """
    Alinha uma imagem multiespectral de 5 bandas usando ECC.

    Parameters
    ----------
    img_norm : np.ndarray
        Imagem normalizada entre 0 e 1.
        Shape esperado: (H, W, 5).

    reference : int, default=2
        Índice da banda utilizada como referência.
        Como Python começa em 0:
    ...
    if not isinstance(img_norm, np.ndarray):
        raise TypeError("img_norm deve ser um numpy.ndarray.")

    if img_norm.ndim != 3:
        raise ValueError(
            f"Esperado array 3D (H, W, 5), recebido {img_norm.shape}."
        )

    if img_norm.shape[2] != 5:
        raise ValueError(
            f"Esperadas 5 bandas, recebido shape {img_norm.shape}."
        )

    if reference not in range(5):
        raise ValueError("reference deve estar entre 0 e 4.")

    # ECC trabalha melhor com float32
    ...
    reference_band = img[:, :, reference]

    img_align = np.empty_like(img)

    # Referência permanece inalterada
    img_align[:, :, reference] = reference_band

    # ---------------------------------------------------------
    # Alinhamento das demais bandas
    # ---------------------------------------------------------
    ...
    try:
        ...
        for band in range(5):
            ...
            moving_band = img[:, :, band]

            # Matriz afim inicial = identidade
            warp_matrix = np.eye(
                2,
                3,
                dtype=np.float32
            ...
            # Estima transformação
            _, warp_matrix = cv2.findTransformECC(
                templateImage=reference_band,
                inputImage=moving_band,
            ...
            img_align[:, :, band] = aligned_band

    except cv2.error:
        return img_norm, "ecc_false"

    # ---------------------------------------------------------
    # Verificações
    # ---------------------------------------------------------

    if not np.all(np.isfinite(img_align)):
        return img_norm, "ecc_false"

    return img_align, "ecc_true"
...
def align_multispectral_keypoints_ransac(
    img_norm: np.ndarray,
    reference: int = 2,
    min_matches: int = 10,
    ...
    min_matches : int
        Número mínimo de matches válidos.

    ratio_threshold : float
        Threshold do Lowe Ratio Test.

    ransac_threshold : float
        Erro máximo de reprojeção, em pixels,
    ...
    tuple
        ...
        Sucesso:
            img_align, "keypoints_ransac_true"

        Falha:
            img_norm, "keypoints_ransac_false"
    """
    ...
    if img_norm.ndim != 3 or img_norm.shape[2] != 5:
        raise ValueError(
            f"Shape esperado: (H, W, 5). "
            f"Recebido: {img_norm.shape}"
        )
    ...
    # Referência permanece intacta
    img_align[:, :, reference] = reference_band

    # ---------------------------------------------------------
    # Conversão para uint8
    #
    # Somente para detecção dos keypoints.
    # A transformação será aplicada no float32 original.
    # ---------------------------------------------------------
    ...
    reference_uint8 = to_uint8(reference_band)
    ...
    if desc_ref is None:
        return img_norm, "keypoints_ransac_false"

    if len(kp_ref) < min_matches:
        return img_norm, "keypoints_ransac_false"
    ...
    try:
        ...
        for band in range(5):
            ...
            kp_mov, desc_mov = sift.detectAndCompute(
                moving_uint8,
                None
            )
            ...
            if not np.all(np.isfinite(affine_matrix)):
                return img_norm, "keypoints_ransac_false"
            ...
            #
            # Importante:
            # keypoints foram encontrados no uint8,
            # mas a transformação é aplicada na banda
            # float32 normalizada original.
            # -------------------------------------------------
    ...
    return img_align, "keypoints_ransac_true"
...
def align_multispectral_homography(
    img_norm: np.ndarray,
    reference: int = 2,
    min_matches: int = 10,
    ratio_threshold: float = 0.75,
    ...
    tuple
        ...
        Sucesso:
            img_align, "homography_true"

        Falha:
            img_norm, "homography_false"
    """
    ...
    if reference not in range(5):
        raise ValueError(
            "reference deve estar entre 0 e 4."
        )
    ...
    # Banda de referência permanece intacta
    img_align[:, :, reference] = reference_band

    # ---------------------------------------------------------
    # Conversão para uint8
    #
    # Somente para encontrar keypoints.
    # ---------------------------------------------------------
    ...
    if desc_ref is None:
        return img_norm, "homography_false"
    ...
    try:
        ...
        for band in range(5):
            ...
            # Matches mínimos
            #
            # Matematicamente uma homografia precisa de pelo
            # menos 4 correspondências, mas mantemos um número
            # maior para aumentar a robustez.
            # -------------------------------------------------
            ...
            # Aplica homografia
            #
            # A transformação é aplicada à banda float32
            # normalizada, e não à versão uint8 usada pelo SIFT.
            # -------------------------------------------------

            aligned_band = cv2.warpPerspective(
                moving_band,
                homography_matrix,
                (width, height),
                flags=cv2.INTER_LINEAR,
    ...
    if not np.all(
        np.isfinite(img_align)
    ):
        return img_norm, "homography_false"

    return img_align, "homography_true"
#======================================================================
import numpy as np
import cv2


def align_multispectral_optical_flow(
    img_norm: np.ndarray,
    reference: int = 2,
    pyr_scale: float = 0.5,
    levels: int = 4,
    ...
    """
    Alinha as 5 bandas de uma imagem multiespectral utilizando
    Dense Optical Flow (Farnebäck).

    Cada banda é alinhada individualmente em relação à banda
    de referência.
    ...
    pyr_scale : float
        Escala utilizada na construção da pirâmide.

    levels : int
        Número de níveis da pirâmide.

    winsize : int
        Tamanho da janela utilizada pelo Optical Flow.

    iterations : int
        Número de iterações por nível da pirâmide.

    poly_n : int
        Tamanho da vizinhança utilizada para aproximação
        polinomial.

    poly_sigma : float
        Desvio padrão do filtro Gaussiano utilizado na
    ...
    tuple
        ...
        Sucesso:
            img_align, "optical_flow_true"

        Falha:
            img_norm, "optical_flow_false"
    """
    ...
    if not isinstance(img_norm, np.ndarray):
        raise TypeError(
            "img_norm deve ser um numpy.ndarray."
        )

    if img_norm.ndim != 3:
        raise ValueError(
            f"Esperado array 3D (H, W, 5). "
            f"Recebido: {img_norm.shape}"
        )

    if img_norm.shape[2] != 5:
        raise ValueError(
            f"Esperadas 5 bandas. "
            f"Recebido: {img_norm.shape}"
        )
    ...
    try:
        ...
        for band in range(5):
            ...
            # Calculamos o fluxo:
            #
            # referência -> banda móvel
            #
            # Assim, para cada pixel da referência sabemos
            # onde buscar o valor correspondente na banda móvel.
            # -------------------------------------------------
            ...
            if flow is None:
                return img_norm, "optical_flow_false"
            ...
            #
            # flow[..., 0] -> deslocamento X
            # flow[..., 1] -> deslocamento Y
            # -------------------------------------------------
    ...
    return img_align, "optical_flow_true"
...
def align_bands_optical_flow(img_norm, reference_band):
    """
    Alinha as bandas de uma imagem multiespectral normalizada usando
    optical flow denso (Farneback) -- permite deformação LOCAL, não
    apenas translação/rotação global.

    Parâmetros
    ----------
    img_norm : ndarray (H, W, B)
        Imagem multiespectral normalizada em [0, 1].
    reference_band : int
        Índice (0-based) da banda usada como referência fixa.

    Retorna
    -------
    (img_align, "optical_flow_true")  -> se todas as bandas foram alinhadas sem erro
    (img_norm,  "optical_flow_false") -> se ocorreu algum erro no processo
    """
    try:
        h, w, n_bands = img_norm.shape
        ref_u8 = (img_norm[:, :, reference_band] * 255).astype(np.uint8)

        img_align = img_norm.copy().astype(np.float32)
        ...
        for b in range(n_bands):
            ...
            mov_u8 = (img_norm[:, :, b] * 255).astype(np.uint8)
            ...
            flow = cv2.calcOpticalFlowFarneback(
                ...
                flags=0,
            )  # flow[y, x] = (dx, dy) que leva mov -> ref

            map_x = (grid_x + flow[..., 0]).astype(np.float32)
            map_y = (grid_y + flow[..., 1]).astype(np.float32)

            aligned = cv2.remap(
                img_norm[:, :, b].astype(np.float32), map_x, map_y,
                interpolation=cv2.INTER_LINEAR,
...
def align_multispectral_bspline(
    img_norm: np.ndarray,
    reference: int = 2,
    mesh_size: tuple = (10, 10),
    number_of_iterations: int = 100,
    learning_rate: float = 5.0,
    histogram_bins: int = 50,
):
    """
    Alinha uma imagem multiespectral de 5 bandas usando
    B-spline Free-Form Deformation (FFD).

    Cada banda é registrada individualmente em relação
    à banda de referência.
    ...
    mesh_size : tuple, default=(10, 10)
        Número de células da malha B-spline em Y e X.

        Valores menores:
            deformação mais suave/global.

        Valores maiores:
            maior capacidade de deformação local.
    ...
    histogram_bins : int
        Número de bins usados pela Mattes Mutual Information.
    ...
    tuple
        ...
        Sucesso:
            img_align, "bspline_true"

        Falha:
            img_norm, "bspline_false"
    """
    ...
    if len(mesh_size) != 2:
        raise ValueError(
            "mesh_size deve ser uma tupla (ny, nx)."
        )
    ...
    try:
        ...
        for band in range(5):
            ...
            # -------------------------------------------------
            # Inicializa transformação B-spline
            # -------------------------------------------------
            ...
            # Métrica
            #
            # Mutual Information é interessante para
            # bandas espectralmente diferentes.
            # ---------------------------------------------
            ...
            aligned_band = sitk.GetArrayFromImage(
                aligned
            ).astype(np.float32)

            if not np.all(
                np.isfinite(aligned_band)
            ):
                return img_norm, "bspline_false"
...
img_5b = load_5b_from_dir(base_dir, file_name)

plot_rgb(img_5b)

#======================================================================

img_norm, norm_params = load_norm_multiband_image(base_dir, file_name, n_bands=5)

print(f'img_norm.shape: {img_norm.shape}')
print(f'img_norm.max(): {img_norm.max()}')
print(f'img_norm.min(): {img_norm.min()}')
...
img_align, status = align_multispectral_ecc(
    img_norm,
    reference=4
)
plot_rgb(img_norm)
plot_rgb(img_align)
plot_two_imgs(img_norm, img_align)
...
img_align, status = align_multispectral_keypoints_ransac(
    img_norm,
    reference=4
)
...
img_align, status = align_multispectral_homography(
    img_norm,
    reference=4
)
...
img_align, status = align_multispectral_optical_flow(
    img_norm,
    reference=4
)
...
#======================================================================
img_align, status = align_bands_optical_flow(
    img_norm,
    reference_band=4
)
