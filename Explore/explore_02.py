...
for especie in especies:  # especie = especies[0]
    ...
    especie_dir = DATA_DIR + f"/{especie}"
    files = os.listdir(especie_dir)

    max_list = []
    min_list = []
    for ith_file in files:  # ith_file = files[0]
        file_dir = especie_dir + f"/{ith_file}"

        with rasterio.open(file_dir) as src:
            band = src.read()
        
        print(f"\n name: {ith_file}")
        print(f"band max: {band.max()}")
        print(f"band min: {band.min()}")

        max_list.append(band.max())
        min_list.append(band.min())
