...
try:
    ...
    from RUN_Preprocessing.test_09_models_1_forward_real.aux_model import *
    from RUN_Preprocessing.test_09_models_1_forward_real.aux_only_models import *
...
def aquecer_cache(diretorio, max_workers=16):
    """Lê todos os .npy do diretório para forçar entrada no page cache do SO."""
    raiz = Path(diretorio)
    arquivos = list(raiz.rglob("*.npy"))
    if not arquivos:
        print(f"[aquecer_cache] Nenhum arquivo encontrado em {raiz}")
        return

    tamanhos = {arq: arq.stat().st_size for arq in arquivos}
    total_bytes = sum(tamanhos.values())

    def ler_bytes_crus(caminho):
        with open(caminho, "rb", buffering=0) as f:
            while f.read(1024 * 1024):
                pass
        return tamanhos[caminho]

    t0 = time.perf_counter()

    with tqdm(total=total_bytes, unit="B", unit_scale=True, unit_divisor=1024,
              desc=f"Aquecendo {raiz.name}") as barra:
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futuros = [executor.submit(ler_bytes_crus, arq) for arq in arquivos]
            for futuro in as_completed(futuros):
                barra.update(futuro.result())

    dt = time.perf_counter() - t0
    print(f"[aquecer_cache] {len(arquivos)} arquivos de {raiz.name} "
          f"({total_bytes / 2**30:.2f} GiB) aquecidos em {dt:.1f}s "
          f"({total_bytes / 2**20 / dt:.1f} MiB/s)")
...
def run_training(config):
    ...
    test_dataset = WeedDataset_Transform(TEST_DIR)
    ...
    if not os.path.isfile(df_metric_val_dir) or not os.path.isfile(df_metric_test_dir):
        ...
        if "model_trained" not in mdl_info["RUN"].keys() or not mdl_info["RUN"]["model_trained"]:
            ...
            print(f" AUGMENTATION: \033[96;95m{AUGMENTATION}\033[0m\n")
            ...
            if "warm_up_data" in config.keys() and config["warm_up_data"]:
                print("\n Aquecendo Dataset \n")
                aquecer_cache(TRAIN_DIR, max_workers=num_workers)
